...
<div class="container">
	...
	<div class="row">
		<?php
			...
			$idTag = get_field('id_tag_societe');
			if(!$idTag){
				$idTag = get_field('id_tag_societe', intval(icl_object_id(get_the_ID(), 'post', true, $other_lang)));
			}
			$magazine_push = null;
			if($idTag){
				$magazine_push = json_decode(file_get_contents(URL_WP_API_BLOG_ARTICLE.'?tags-api='.$idTag.'&per_page=1&lang='.ICL_LANGUAGE_CODE),true);
			}
