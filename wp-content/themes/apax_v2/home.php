	...
	<div id="wrap-list-article">
		<div class="container">
			<div class="row row-mini">
				<?php global $wpdb;
				$query = "SELECT DISTINCT YEAR(wposts.post_date) as year FROM $wpdb->posts wposts, ".$wpdb->prefix."icl_translations wicl_translations WHERE wposts.post_type = 'post' AND wposts.post_status = 'publish' AND wicl_translations.element_id = wposts.ID AND wicl_translations.language_code = 'en' ORDER BY year DESC";
				...
				<div class="grid">
					...
					<div class="grid-item col-md-4 col-lg-3 col-sm-6 <?php echo "cat-item-".get_the_date("Y"); ?>">
						<article>
							<div class="block-article">
								<a href="<?php echo get_permalink(); ?>"><span class="date"><?php echo str_replace(" ","<br/>",get_the_date()); ?></span></a>
								<?php $origine = get_the_title();
								$cut = substr($origine,0,120);
								if ($cut < $origine) $cut .= "..."; ?>
								<h2><a href="<?php echo get_permalink(); ?>"><?php echo $cut; ?></a></h2>
								<a class="plus" href="<?php echo get_permalink(); ?>"><?php _e("Read more","apax"); ?></a>
							</div>
						</article>
					</div>
