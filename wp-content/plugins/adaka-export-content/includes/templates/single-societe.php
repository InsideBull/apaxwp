		...
		<div class="col-lg-offset-1 col-lg-7 col-md-8">
			<div class="post">
				<div class="post-content">
					<div>
						<?php 
							$thumb = wp_get_attachment_image_src( get_post_thumbnail_id(), 'image-push' );
							...
							echo '<div class="entrepreneur-wrapper clearfix">';
								...
								echo '</div>';
							echo '</div>';
							echo '<div class="adaka-collapse content">';
					...
					<div class="chiffres-societe">
						<?php if(get_field('chiffre_affaires') && get_field('show_in_historical')[0] != 'Oui'): ?>
							<div class="chiffre-affaires">
									<?= get_field('chiffre_affaires_prefixe') ?><span class="num animated-number"><?= $chiffre_affaires ?></span><?= get_field('chiffre_affaires_suffixe') ?><br>
								<span class="text">
						...
						<?php endif; ?>
						<div class="small">
							<div class="secteur"><?= __("Sector","apax") ?><?= $lang == "fr" ? ' ' : '' ?>: <?= get_the_category()[0]->name; ?></div>
							<span class="date_investissement"><?= __("Investment date","apax") ?><?= $lang == "fr" ? ' ' : '' ?>: <?= $date_dinvestissement_societe ?></div>
							<?= $majoritaire != "" && $majoritaire ? '<div class="majoritaire">'.__("Apax position","apax").($lang == "fr" ? ' ' : '').': '.$majoritaire.'</div>' : ''; ?>
							<?= !empty($poles) && $poles ? '<div class="activities">'.__((count($poles)>1?"Activities":"Activity"),"apax").($lang == "fr" ? ' ' : '').': '.implode(', ', $poles).'</div>' : ''; ?>
						</div>
					</div>
					...
					if ($history && count($history)>0) {
						echo '<div class="content_history">';
							...
							echo '<div class="online-societe">';
								...
								if ($history[$i]["txt_internationalisation"] != "" || $history[$i]["txt_build-ups"] != "" || $history[$i]["txt_digitial"] != ""){
									...
									echo $history[$i]["txt_internationalisation"] != "" ? '<div role="tabpanel" class="tab-pane active"><div class="table"><div class="ico"><img src="'.get_bloginfo("template_url").'/img/ico-internationnal.png" alt="" /></div><div>'.$history[$i]["txt_internationalisation"].'</div></div></div>' : '';
									...
									echo '</ul>';
									echo $history[$i]["txt_build-ups"] != "" ? '<div role="tabpanel" class="tab-pane'.($history[$i]["txt_internationalisation"] == "" ? ' active' : '').'"><div class="table"><div class="ico"><img src="'.get_bloginfo("template_url").'/img/ico-buildups.png" alt="" /></div><div>'.$history[$i]["txt_build-ups"].'</div></div></div>' : '';
									...
									echo '</ul>';
									echo $history[$i]["txt_digitial"] != "" ? '<div role="tabpanel" class="tab-pane'.($history[$i]["txt_build-ups"] == "" && $history[$i]["txt_internationalisation"] == "" ? ' active' : '').'"><div class="table"><div class="ico"><img src="'.get_bloginfo("template_url").'/img/ico-digital.png" alt="" /></div><div>'.$history[$i]["txt_digitial"].'</div></div></div>' : '';
						...
						echo '</div>';
							if(count($history)>1){
								echo '<div class="content_history adaka-collapse">';
